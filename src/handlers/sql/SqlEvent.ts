...
export default class SqlEvent {
  ...
  public async createEvent(eventName: string, eventDate: string, isCta: boolean) {
    ...
    try {
      ...
      await conn.query(
        `INSERT INTO events (name, date, is_cta) VALUES (${conn.escape(eventName)}, ${conn.escape(eventDate)}, ${
          isCta ? '1' : '0'
        })`
      );
      const rows = await conn.query(
        `SELECT id FROM events WHERE name = ${conn.escape(eventName)} AND date = ${conn.escape(eventDate)}`
      );
      if (rows && rows[0]) {
      ...
      returnValue = -1;
      Logger.exception("SQL: Couldn't create event", err, WARNINGLEVEL.WARN);
    } finally {
  ...
  public async deleteEvent(eventName: string, eventDate: string) {
    ...
    try {
      ...
      returnValue = false;
      Logger.exception("SQL: Couldn't delete event", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async getEventId(eventName: string, eventDate: string) {
    ...
    try {
      ...
      Logger.exception("SQL: Couldn't retrieve event", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async findEvents(timestamp: string, isClosed: boolean, isFormed: boolean, isCta: boolean) {
    ...
    try {
      ...
      const rows = await conn.query(
        `SELECT id FROM events WHERE date < ${conn.escape(timestamp)}${
          isClosed !== undefined ? ` AND is_closed = ${isClosed ? '1' : '0'}` : ''
        }${isFormed !== undefined ? ` AND is_formed = ${isFormed ? '1' : '0'}` : ''}${
          isCta !== undefined ? ` AND is_cta = ${isCta ? '1' : '0'}` : ''
        }`
      );
      if (rows) {
      ...
      returnValue = [];
      Logger.exception("SQL: Couldn't find events", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async findEventObjects(timestamp: string): Promise<{ id: number; date: number }[]> {
    ...
    try {
      ...
      const rows = await conn.query(
        `SELECT id, date FROM events WHERE date < ${conn.escape(
          timestamp
        )} AND is_closed = 1 AND is_formed = 1 AND is_cta = 1`
      );
      if (rows) {
        for (const row of rows) {
      ...
      returnValue = [];
      Logger.exception("SQL: Couldn't find events (objects)", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async updateEventFlags(eventId: number, isClosed: boolean, isFormed: boolean, isCta: boolean) {
    ...
    try {
      ...
      returnValue = false;
      Logger.exception("SQL: Couldn't update event", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async getEvents(includeClosed: boolean) {
    ...
    try {
      ...
      returnValue = [];
      Logger.exception("SQL: Couldn't retrieve events", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
  ...
  public async isCtaEvent(eventId: number) {
    ...
    try {
      ...
      returnValue = false;
      Logger.exception("SQL: Couldn't retrieve event (CTA-check)", err, WARNINGLEVEL.WARN);
    } finally {
      if (conn) await conn.end();
    }
    return returnValue;
