...
import { Logger, MessageHandler } from 'discord.ts-architecture';
...
export default class PartyHandler {
  ...
  public static async formCategories(users: { userId: string; date: number }[], guild: Guild) {
    ...
    Logger.info('PartyHandler: Retrieving Discord Users');
    ...
    Logger.info('PartyHandler: Calculating Party Settings');
    const numberOfParties = Math.ceil((discordUsers.length + 1) / 20.0);
    let bms = Math.max(
      PartyHandler.BmSettings.Minimum,
      Math.floor(numberOfParties / PartyHandler.BmSettings.BmEveryParty) * PartyHandler.BmSettings.BmPerParty
    );
    bms = Math.min(
      bms,
      discordUsers.filter((user) => user.roles.find((role) => role === 'Battlemount') !== undefined).length
    );
    const parties: { userId: string; date: number; role: string }[][] = Array.from(
      new Array(numberOfParties),
      () => []
    );

    let missingPlayers = 19;
    Logger.info('PartyHandler: Number of Parties:', numberOfParties);
    for (let i = 0; i < numberOfParties; i++) {
      Logger.info('PartyHandler: Forming Party ' + (i + 1) + '/' + numberOfParties);
      // Logger.Log("================== DISCORD USERS =====================");
      // Logger.Log(discordUsers.length);
      // Logger.Log("Party Index: " + i);
      // Logger.Log(parties[i]);
      // add bms
      Logger.info('PartyHandler: Adding Battlemounts');
      ...
      Logger.info('PartyHandler: Adding Required Roles');
      ...
      Logger.info('PartyHandler: Adding Global Roles');
      ...
      Logger.info('PartyHandler: Adding Random Roles');
      for (const globalRole of PartyHandler.GlobalRoles) {
    ...
    // Logger.Log("================== PARTIES =====================");
    // Logger.Log(parties);
    Logger.info('PartyHandler: Forming Discord Messages');
    ...
    for (let party of parties) {
      ...
      const partyCategories = MessageHandler.splitInCategories(
        partyLines,
        LanguageHandler.replaceArgs(LanguageHandler.language.handlers.party.partyTitle, [partyIndex.toString()])
      );
      categories.push(...partyCategories);
      partyIndex++;
    }
    Logger.info('PartyHandler: Party Forming Complete');
    return categories;
  }
  ...
  private static addUserToParty(
    discordUsers: { userId: string; date: number; roles: string[] }[],
    parties: { userId: string; date: number; role: string }[][],
    partyIndex: number,
    ...
    for (const user of clone) {
      ...
      for (const role of roles) {
        ...
        if (
          !role.MaximumPerParty ||
          role.MaximumPerParty >
            parties[partyIndex].filter((p) => p.role === role.PriorityRole || p.role === role.RoleName).length
        ) {
          foundRole = role;
          break;
        }
  ...
  private static addUserToPartyOld(
    discordUsers: { userId: string; date: number; roles: string[] }[],
    parties: { userId: string; date: number; role: string }[][],
    partyIndex: number,
    globalRole: GlobalRole
