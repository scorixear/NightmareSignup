...
export default class CheckAttendance extends CommandInteractionModel {
  constructor() {
    const commandOptions: any[] = [
      new SlashCommandIntegerOption()
        .setName('event-count')
        .setDescription(LanguageHandler.language.commands.attendance.event_count_desc)
        .setRequired(false),
      new SlashCommandStringOption()
        .setName('event-name')
        .setDescription(LanguageHandler.language.commands.attendance.event_name_desc)
        .setRequired(false),
      new SlashCommandStringOption()
        .setName('event-date')
        .setDescription(LanguageHandler.language.commands.attendance.event_date_desc)
        .setRequired(false),
      new SlashCommandStringOption()
        .setName('event-time')
        .setDescription(LanguageHandler.language.commands.attendance.event_time_desc)
        .setRequired(false)
    ];
    super(
      'checkattendance',
  ...
  override async handle(interaction: ChatInputCommandInteraction) {
    ...
    if (eventName || eventDate || eventTime) {
      if (!eventName || !eventDate || !eventTime) {
        Logger.info('CheckAttendance: Missing Parameters');
        ...
        try {
          ...
          if (isNaN(eventTimestamp)) {
            Logger.info('CheckAttendance: Invalid Date');
            await interaction.reply({
          ...
          Logger.exception('CheckAttendance: Date Parsing crash', err, WARNINGLEVEL.WARN);
          await interaction.reply({
        ...
        if (!eventId) {
          Logger.info('CheckAttendance: Event not found');
          await interaction.reply({
        ...
        description = LanguageHandler.replaceArgs(LanguageHandler.language.commands.attendance.description_event, [
          eventName,
          eventDate,
          eventTime
        ]);
      ...
      description = LanguageHandler.replaceArgs(LanguageHandler.language.commands.attendance.success.limit_desc, [
        count.toString()
      ]);
      limit = 1;
    } else {
    ...
    for (const pair of userCounts) {
      if (pair[1].max - pair[1].reacted > limit) {
        ...
        lines.push(
          `${member.nickname ? member.nickname : member.user.username}: ${pair[1].max - pair[1].reacted} / ${
            pair[1].max
          }`
        );
      }
    ...
    const categories = MessageHandler.splitInCategories(
      lines,
      LanguageHandler.language.commands.attendance.success.list
    );
    Logger.info('CheckAttendance: Success');
